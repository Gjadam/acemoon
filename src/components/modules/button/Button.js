...
export default function Button({ type, text, children, isSubmitType, onClick, isDisabled, isWidthFull }) {
    return (
        type === 'simple' ? (
            ...
            <button type={isSubmitType ? 'submit' : 'button'} className={`  flex justify-center items-center gap-3 text-white  ${isDisabled ? "opacity-50 bg-secondary cursor-not-allowed" : "bg-rose-500 hover:bg-inherit hover:text-rose-500"} ${isWidthFull ? "w-full" : "w-full md:min-w-40 md:max-w-44"}   text-center py-2 rounded-3xl overflow-hidden  z-10  relative after:absolute after:left-0 after:bottom-0 after:w-full after:bg-[rgba(225,225,225,225.0.5)] after:h-0 after:hover:h-full after:-z-10 after:transition-all after:duration-300 before:absolute before:left-0 before:-bottom-1 before:hover:bottom-full before:w-full before:h-px before:bg-white before:transition-all before:duration-300 transition-colors duration-500`} disabled={isDisabled} onClick={onClick}>
                <span className="">{text}</span>
                {
                    children && children
                }
